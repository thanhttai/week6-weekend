...
const CartTotals = ({cart,handleCheckOut }) => {
const myUser = useSelector(state=> state?.user?.userLocal)
let total_amount;
if(cart){
...
const shipping_fee = 5.34
  ...
  return <Wrapper>
    <div>
      ...
      {myUser? <button onClick={handleCheckOut} className="btn">Check Out</button> : <Link to="/login" className="btn" type="button">login</Link>}
    </div>
  </Wrapper>
}
